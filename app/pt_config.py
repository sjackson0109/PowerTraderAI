...
from pt_errors import ConfigurationError, ValidationError, handle_errors, ErrorSeverity
...
class TradingConfig:
    ...
    analysis_timeframes: List[str] = field(default_factory=lambda: ["5m", "15m", "1h", "4h"])
...
class ExchangeConfig:
    ...
    supported_pairs: List[str] = field(default_factory=lambda: ["BTC-USD", "ETH-USD", "LTC-USD"])
...
class UIConfig:
    ...
    notification_types: List[str] = field(default_factory=lambda: ["trades", "errors", "alerts"])
...
class ConfigValidator:
    ...
    def validate_exchange_config(config: ExchangeConfig) -> List[str]:
        ...
        if not config.primary_pair or not ConfigurationValidator.validate_symbol(config.primary_pair):
...
class ConfigurationManager:
    ...
    def _load_config_file(self, file_path: Path, config_instance: Any, 
    ...
    def _apply_environment_overrides(self) -> None:
        ...
        if env_file.exists():
            try:
                ...
                self.logger.info(f"Applied environment overrides for: {self.environment}")
    ...
    def _validate_all_configs(self) -> None:
        ...
        validation_errors.extend(ConfigValidator.validate_exchange_config(self.exchange))
        validation_errors.extend(ConfigValidator.validate_security_config(self.security))
