...
class ValidationError(PowerTraderError):
    ...
    def __init__(self, message: str, field_name: str = None, field_value: Any = None,
                 severity: ErrorSeverity = ErrorSeverity.LOW, context: Dict[str, Any] = None):
        context = context or {}
        if field_name:
...
class ConfigurationError(PowerTraderError):
    ...
    def __init__(self, message: str, config_key: str = None, config_file: str = None,
                 severity: ErrorSeverity = ErrorSeverity.HIGH, context: Dict[str, Any] = None):
        context = context or {}
        if config_key:
...
class DataError(PowerTraderError):
    ...
    def __init__(self, message: str, data_source: str = None, expected_format: str = None,
                 severity: ErrorSeverity = ErrorSeverity.MEDIUM, context: Dict[str, Any] = None):
        context = context or {}
        if data_source:
...
class ErrorHandler:
    ...
    def handle_error(self, error: Exception, context: Dict[str, Any] = None) -> ErrorReport:
        ...
        error_report = ErrorReport(
            ...
            recovery_suggestion=self.recovery_suggestions.get(category, "Contact support for assistance.")
        )
    ...
    def _classify_error(self, error: Exception) -> ErrorCategory:
        ...
        elif 'file' in error_str or 'ioerror' in error_type or 'permission' in error_str:
        ...
        elif 'validation' in error_str or 'invalid' in error_str or 'value' in error_type:
    ...
    def _determine_severity(self, error: Exception, category: ErrorCategory) -> ErrorSeverity:
        ...
        if any(word in error_str for word in ['critical', 'fatal', 'shutdown', 'crash']):
    ...
    def _generate_user_message(self, category: ErrorCategory, error_message: str) -> str:
        ...
        return category_messages.get(category, "An unexpected error occurred. Please try again or contact support.")
    ...
    def _log_error(self, error_report: ErrorReport) -> None:
        ...
        if error_report.traceback_str and error_report.severity in [ErrorSeverity.HIGH, ErrorSeverity.CRITICAL]:
            self.logger.debug(f"[{error_report.error_id}] Traceback:\\n{error_report.traceback_str}")
