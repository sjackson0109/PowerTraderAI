...
from pt_live_monitor import Alert, LiveMonitor
...
class TradingControlPanel(ttk.Frame):
    ...
    def _create_account_section(self, parent):
        ...
        ttk.Label(account_frame, text="Account Type:").grid(
        ...
        ttk.Label(
            account_frame, textvariable=self.account_status_var, foreground="orange"
    ...
    def _create_trading_section(self, parent):
        ...
        ttk.Label(quick_frame, text="Symbol:").grid(
        ...
        self.toggle_trading_btn = ttk.Button(
            control_frame, text="Enable Trading", command=self._toggle_trading
        )
    ...
    def _create_monitoring_section(self, parent):
        ...
        ttk.Label(control_frame, textvariable=self.monitoring_status_var).pack(
            side="left"
        )

        self.toggle_monitoring_btn = ttk.Button(
            control_frame, text="Start Monitoring", command=self._toggle_monitoring
        )
        ...
        alerts_scroll = ttk.Scrollbar(
            alert_frame, orient="vertical", command=self.alerts_text.yview
        )
    ...
    def _create_status_section(self, parent):
        ...
        self.refresh_btn = ttk.Button(
            refresh_frame, text="Refresh Status", command=self._refresh_status
        )
        ...
        try:
            ...
            if account_type == "Paper Trading":
                ...
                self.risk_manager = RiskManager(
                    limits, portfolio_value=float(initial_balance)
                )
                ...
                self.account_status_var.set(
                    f"Paper Trading Account Initialized: ${initial_balance:,.2f}"
                )
                self._update_status(
                    f"Account initialized successfully at {datetime.now().strftime('%H:%M:%S')}"
                )
    ...
    def _execute_buy(self):
        ...
        try:
            ...
            order_id = self.paper_account.place_order(
                symbol, OrderType.MARKET, OrderSide.BUY, quantity
            )
            self._update_status(
                f"Buy order placed: {quantity} {symbol} (Order: {order_id[:8]}...)"
            )
    ...
    def _execute_sell(self):
        ...
        try:
            ...
            order_id = self.paper_account.place_order(
                symbol, OrderType.MARKET, OrderSide.SELL, quantity
            )
            self._update_status(
                f"Sell order placed: {quantity} {symbol} (Order: {order_id[:8]}...)"
            )
            self._refresh_status()
...
class RiskManagementPanel(ttk.Frame):
    ...
    def _setup_ui(self):
        ...
        ttk.Label(pos_frame, text="Max Position Size (%):").grid(
            row=0, column=0, sticky="w", padx=(0, 10)
        )
        ...
        ttk.Label(pos_frame, text="Max Daily Loss (%):").grid(
        ...
        for i, (name, desc) in enumerate(scenarios):
            ttk.Radiobutton(
                scenario_frame,
                text=f"{name} - {desc}",
                variable=self.risk_scenario_var,
                value=name,
            ).grid(row=i, column=0, sticky="w", pady=2)

        # Apply button
        apply_btn = ttk.Button(
            main_frame, text="Apply Risk Settings", command=self._apply_settings
        )
...
class CostAnalysisPanel(ttk.Frame):
    ...
    def _setup_ui(self):
        ...
        cost_frame = ttk.LabelFrame(
            main_frame, text="Monthly Cost Breakdown", padding="5"
        )
        ...
        cost_scroll = ttk.Scrollbar(
            cost_frame, orient="vertical", command=self.cost_text.yview
        )
