...
class MonitoringDashboard:
    ...
    def display_header(self):
        ...
        print(
            f"{Back.BLUE}{Fore.WHITE} PowerTraderAI+ - Live Monitoring Dashboard {Style.RESET_ALL}"
        )
        print(
            f"Runtime: {runtime} | Last Update: {datetime.now().strftime('%H:%M:%S')}"
        )
        print("=" * 80)
    ...
    def display_system_status(self):
        ...
        # Trading status
        trading_status = (
            "ACTIVE" if not self.risk_manager.is_trading_halted() else "HALTED"
        )
        ...
        emergency_status = (
            "ACTIVE" if self.risk_manager.emergency_stop_active else "INACTIVE"
        )
        ...
        # Risk level
        print(
            f"Risk Level: {self.format_risk_level(self.risk_manager.current_risk_level)}"
        )

        # Cost tier
        print(
            f"Cost Tier: {Fore.CYAN}{self.cost_manager.tier.value.title()}{Style.RESET_ALL}"
        )
        print()
    ...
    def display_portfolio_summary(self, portfolio_value: float):
        ...
        if (
            hasattr(self.risk_manager, "initial_portfolio_value")
            and self.risk_manager.initial_portfolio_value
        ):
            ...
            print(
                f"Total Return: {self.format_currency(change)} ({self.format_percentage(change_pct)})"
            )
        ...
        today_history = [
            ...
            if p["timestamp"] > datetime.now() - timedelta(days=1)
        ]
        if today_history:
    ...
    def display_risk_metrics(self):
        ...
        if self.risk_manager.position_tracker:
            ...
            for symbol, position in self.risk_manager.position_tracker.items():
                ...
                print(
                    f"  {symbol}: {position['quantity']:.4f} @ {self.format_currency(position['avg_price'])} = {self.format_currency(value)}"
                )
    ...
    def display_performance_metrics(self, portfolio_value: float):
        ...
        current_drawdown = (
            (portfolio_value - peak_value) / peak_value * 100 if peak_value > 0 else 0
        )
    ...
    def display_alerts_and_warnings(self):
        ...
        if (
            ...
            monthly_burn_rate = (
                monthly_costs.total_monthly / self.risk_manager.initial_portfolio_value
            )
            if monthly_burn_rate > 0.05:  # More than 5% monthly burn
                warnings.append(
                    f"HIGH BURN RATE: {monthly_burn_rate:.1%} monthly cost burn rate"
                )
    ...
    def update_data(self, portfolio_value: float):
        ...
        self.portfolio_history.append(
            {"timestamp": datetime.now(), "value": portfolio_value}
        )

        # Keep only last 24 hours of data
        cutoff_time = datetime.now() - timedelta(hours=24)
        self.portfolio_history = [
            p for p in self.portfolio_history if p["timestamp"] > cutoff_time
        ]
        ...
        if risk_status["warnings"]:
            for warning in risk_status["warnings"]:
                self.alert_history.append(
                    {"timestamp": datetime.now(), "type": "warning", "message": warning}
                )

    def run_dashboard(self):
        """Run the live monitoring dashboard"""
        print(
            f"{Fore.CYAN}Starting PowerTraderAI+ Monitoring Dashboard...{Style.RESET_ALL}"
        )
        ...
        try:
            while True:
                ...
                print(
                    f"{Fore.CYAN}Refreshing every {self.refresh_interval} seconds...{Style.RESET_ALL}"
                )
