...
class RiskManager:
    ...
    def _check_portfolio_risk(self, metrics: PortfolioMetrics):
        ...
        if metrics.daily_pnl < 0:
            ...
            if daily_loss_pct >= self.limits.max_daily_loss:
                ...
                daily_loss_pct >= self.risk_thresholds["portfolio_drawdown"]["critical"]
            ):
                self._trigger_alert(
                    RiskLevel.CRITICAL,
                    RiskEventType.PORTFOLIO_DRAWDOWN,
                ...
                daily_loss_pct >= self.risk_thresholds["portfolio_drawdown"]["warning"]
            ):
                self._trigger_alert(
                    RiskLevel.WARNING,
                    RiskEventType.PORTFOLIO_DRAWDOWN,
    ...
    def calculate_position_size(
        self, account_value: float, risk_per_trade: float = 0.01
    ) -> float:
        """
    ...
    def get_risk_summary(self) -> Dict:
        ...
        return {
            ...
            "highest_recent_alert": max(
                [a.level.value for a in recent_alerts], default="none"
            ),
            "portfolio_value": self.portfolio_value,
            "risk_limits": {
