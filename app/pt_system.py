...
from pt_config import ConfigurationManager, ExchangeConfig, TradingConfig
from pt_errors import ErrorHandler, PowerTraderError, error_handler
from pt_logging import PowerTraderLogger, configure_logging, shutdown_logging
from pt_performance import (
    PerformanceMonitor,
    cleanup_performance_monitoring,
    performance_monitor,
)
from pt_testing import ComponentTester, TradingStrategyTester, run_comprehensive_tests
from pt_utils import PerformanceTimer, SafeFileHandler
...
class PowerTraderSystem:
    ...
    def _initialize_configuration(self) -> None:
        ...
        self.config_manager = ConfigurationManager(
            config_dir=self.config_dir, enable_hot_reload=True
        )
    ...
    def _initialize_logging(self) -> None:
        ...
        self.logger.configure(
            log_level="INFO", log_file=str(log_file), json_output=True
        )
        ...
        try:
            ...
            if self.logger:
                self.logger.log_error_with_context(
                    "Failed to update logging configuration", error=e
                )
    ...
    def _update_logging_configuration(self) -> None:
        ...
        if current_level != new_level:
            ...
            self.logger.get_logger().info(
                f"Log level changed to {system_config.log_level}"
            )
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            if self.logger:
                self.logger.get_logger().info(
                    "PowerTraderAI+ system started successfully"
                )
        ...
        def health_monitor():
            while self.is_running:
                try:
                    ...
                    if self.logger:
                        self.logger.log_error_with_context(
                            "Health monitoring error", error=e
                        )

        self.health_check_thread = threading.Thread(
            target=health_monitor, daemon=True, name="HealthMonitor"
        )
    ...
    def _update_system_status(self) -> None:
        ...
        try:
            ...
            if self.performance_monitor:
                self.status.performance_summary = (
                    self.performance_monitor.get_system_summary()
                )
            ...
            if self.logger:
                self.logger.log_error_with_context(
                    "Failed to update system status", error=e
                )
    ...
    def run_trading_strategy_test(
        self, strategy_func: Callable, periods: int = 100
    ) -> Dict[str, Any]:
        """
        ...
        try:
            with PerformanceTimer(
                "strategy_test", self.logger.get_logger() if self.logger else None
            ):
                ...
                result = tester.run_strategy_test(
                    strategy_func, market_data, "User Strategy Test"
                )
            ...
            if self.error_handler:
                self.error_handler.handle_error(
                    e, {"operation": "strategy_test", "periods": periods}
                )
            raise
