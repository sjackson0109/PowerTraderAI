...
• Improved connection stability
    ...
    def download_update(
        self, update_info: Dict[str, Any], progress_callback=None
    ) -> Optional[str]:
    ...
    def should_check_for_updates(self) -> bool:
        ...
        try:
            ...
            check_interval = timedelta(
                hours=self.settings.get("check_interval_hours", 24)
            )
...
class UpdateDialog(tk.Toplevel):
    ...
    def _setup_ui(self):
        ...
        title_label = ttk.Label(
            header_frame, text="🚀 Update Available!", font=("Arial", 16, "bold")
        )
        title_label.pack()
        ...
        version_frame = ttk.LabelFrame(
            main_frame, text="Version Information", padding="10"
        )
        ...
        ttk.Label(
            version_frame, text=f"New Version: {self.update_info['version']}"
        ).pack(anchor="w")

        size_mb = self.update_info.get("size", 0) / 1024 / 1024
        ttk.Label(version_frame, text=f"Download Size: {size_mb:.1f} MB").pack(
            anchor="w"
        )
        ...
        notes_text.insert(
            "1.0", self.update_info.get("release_notes", "No release notes available.")
        )
        ...
        ttk.Button(button_frame, text="Update Now", command=self._accept).pack(
            side="right"
        )
        ttk.Button(
            button_frame, text="Remind Me Later", command=self._remind_later
...
def check_and_apply_updates(parent_window=None) -> bool:
    ...
    try:
        ...
        try:
            ...
            download_path = update_manager.download_update(
                update_info, progress_callback
            )
...
if __name__ == "__main__":
    ...
    ttk.Button(
        root, text="Check for Updates", command=lambda: check_and_apply_updates(root)
    ).pack(pady=50)
