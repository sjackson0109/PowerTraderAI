...
class SecureCredentialManager:
    ...
    def _get_machine_password(self) -> str:
        ...
        machine_info = f"{os.environ.get('COMPUTERNAME', '')}{os.environ.get('USERNAME', '')}"
    ...
    def decrypt_credentials(self) -> Optional[Tuple[str, str]]:
        ...
        try:
            if not (os.path.exists(self.encrypted_key_file) and 
                   os.path.exists(self.encrypted_secret_file) and
...
def get_credentials() -> Optional[Tuple[str, str]]:
    """
    Get API credentials, handling both encrypted and legacy plaintext formats.
